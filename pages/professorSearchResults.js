      ...
      {this.props.students.map((student, index) => (
        <Card className={classes.studentCard}>
          <CardContent>
            ...
            <CardActions>
              <Button onClick={() => Router.push(`/professorMore?user=${this.props.netid}&other=${student.netid}`)} size="small"> More </Button>
            </CardActions>
          </CardContent>
        </Card>
        ))}
...
class StudentSearchPage extends React.Component {
  ...
  static async getInitialProps({ query }) {
    ...
    const search = await fetch('http://35.237.162.74:3000/api/v1/search?type=Professor&term=' + query.term);
